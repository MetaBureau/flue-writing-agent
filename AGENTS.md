...
- Flue workflow: `npx flue run src/agents/writer.ts` or `writeWithFlue` in `src/agents/run.ts`. Fresh `/api/write` and `deno task eval` use that. `AgentRunError` on a failed `read()`. Dropping the `/api/write` stream, or starting a new write, aborts Flue (`agent.abort()`) and in-flight writer/Tavily HTTP. A cancelled run does not persist a recovered essay.
- CLI: `deno task start "<topic>"` (or `deno task write`) calls `writeWithFlue`. `src/workflow.ts` is leftover. Do not add stages there.
