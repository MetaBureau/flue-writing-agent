...
- Notes are structured from full extracted articles. Extract failure or empty extract falls back to search snippets. Each note keeps id, URL, author, outlet, date, stance, and quoted claims. The outlet must match the URL's domain; a brand mentioned on the page does not replace it. Content farms are blocked with essay mills and social hosts. Writers see those notes with attribution. The draft cites `[n3]`. Paraphrase; quoted words stay under 15% of the body. The harness rejects an uncited figure or quote, a missing note id, unquoted copying of an 8-word phrase that has at least three distinctive words, and a quote share over 15%. A year needs a cite only when it appears in the notes. After two revise passes, leftover copy is quoted in place only if that stays under the quote budget; leftover harness problems save as a critic warning. Plan at most three sections at 500–700 words, four through 1500, five through 2500, six above that. Do not label sections Introduction or Conclusion. Short essays are continuous prose, not a heading per paragraph. Opening, close, and body may add argument, interpretation, examples, transitions, and widely known general knowledge. Do not invent statistics, studies, quotes, or sources. Do not present a weak source such as a forum post or a TIL as research
