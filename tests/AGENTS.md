...
- Assert attributed notes keep outlet, URL, stance, and quotes; draft prompts
  cite `[n3]` and do not say to weave unused notes; the critic prompt does not
  invent a wit rule; the frog brief is verbatim in stage prompts
- Assert the citation harness rejects an uncited figure or quote and a missing
  note id, and does not require a cite for a year the notes do not mention; the
