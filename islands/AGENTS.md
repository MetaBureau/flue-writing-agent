...
- Read the event stream from `/api/write` (`writeWithFlue`). An `AbortController`
  cancels that fetch on unmount and on a new submit. An abort is not a writer
  failure
...
- Mark each `WRITE_STAGES` entry active, done, warning, or error. That list
  is Brief, Research, Plan, Draft, Critic. The first event is Brief active,
  Writer running. Skipped research is `step-warning`, same as a leftover critic
...
- Style names come from the route. Writer radios are `WRITER_OPTIONS` in `src/providers.ts`, default `anthropic/claude-sonnet-5`. The form posts that option's provider and model. It does not post a checker. The selected writer runs notes, plan, and draft. The critic is Gemini 3.5 Flash unless the API passes `checkModel`
...
The form is exercised by using `deno task dev`, or `deno task serve` after
`deno task build`, not by a unit test. CI smokes the built form HTML.
