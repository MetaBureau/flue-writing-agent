...
- Mark each `WRITE_STAGES` entry active, done, warning, or error
- Show `error` in an alert. Keep a synthesis fallback as `alert-warning` and
  `step-warning`; do not treat it as a failed run. Keep notes, each draft, synthesis, and the essay as
