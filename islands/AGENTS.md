...
- Mark each `WRITE_STAGES` entry active, done, warning, or error. That list
  starts with Brief and includes Brief check before Fact-check
- Show `error` in an alert. Keep a synthesis fallback, a skipped brief check,
  and a failed brief check that keeps the styled essay as `alert-warning` and
  `step-warning`; do not treat them as a failed run. Keep notes, each draft, synthesis, a brief revision when one is kept, and the essay as
  pieces, ordered notes → `draft:*` → synthesis → essay via `pieceRank`. View opens a DaisyUI
...
- Style, provider, writer model, and checker options come from the route, not a second catalog. The checker list is the HaiMaker picker, defaulting to `google/gemini-3.1-flash-lite`
