...
async function searchNotes(query: string): Promise<string> {
  ...
  if (!research.text) {
    return "No sources. Write only from the user topic, and say the search returned nothing.";
  }
...
async function readPage(url: string): Promise<string> {
  ...
  const text = raw.replace(/<script[\s\S]*?<\/script>/gi, " ").replace(
    /<style[\s\S]*?<\/style>/gi,
    " ",
...
function modelFor(data: WriterData): string {
  return data?.provider === "haimaker"
    ? MODEL_FOR_PROVIDER.haimaker
    : MODEL_FOR_PROVIDER.mercury;
}
...
function slug(topic: string): string {
  return topic.trim().split(/\s+/).slice(0, 6).join(" ").toLowerCase().replace(
    /[^a-z0-9]+/g,
    "-",
  ).replace(/^-+|-+$/g, "");
}
...
export function Writer() {
  ...
  useTool({
    name: "search_notes",
    description:
      "Search for claims about the topic. Returns source snippets with ads, biographies, and page chrome removed. Call this once before writing.",
  ...
  useTool({
    name: "read_page",
    description:
      "Fetch one documentation URL and return its text. Prefer this when the user gives a URL.",
  ...
  useTool({
    name: "save_essay",
    description:
      "Save the finished essay. Call this once after the essay is written, then stop.",
    input: v.object({
