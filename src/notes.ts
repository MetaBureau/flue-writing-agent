...
const GENERIC_QUERY_WORDS = new Set(["search", "writing", "agents", "about", "blog", "words", "using", "write", "posts"]);
...
const FURNITURE = /\b(subscribers?|likes|views|bootcamp|colombo|lkr|seats|sign in|sign up|copyright|lukrembo|music track|followers|unknown user|authored|written by)\b|press enter or click|view image in full size|reach out/i;
...
export function claimText(text: string): string {
  ...
  return sentences.filter((sentence) => sentence && !FURNITURE.test(sentence)).join(" ");
}

function excerpt(text: string): string {
  return claimText(text).split(" ").filter(Boolean).slice(0, EXCERPT_WORDS).join(" ");
}

export function sourceNotes(hits: SearchHit[]): string {
  return hits.filter((hit) => hit.url && countWords(hit.content) >= 15).slice(0, MAX_SOURCES).map((hit) =>
    `Source: ${hit.title}\nURL: ${hit.url}\n${excerpt(hit.content)}`
  ).join("\n\n");
}
...
export function relevantToQuery(query: string, hit: SearchHit): boolean {
  const distinctive = contentWords(query).filter((word) => word.length > 5 && !GENERIC_QUERY_WORDS.has(word));
...
export function selectHits(query: string, hits: SearchHit[]): SearchHit[] {
  return hits.filter((hit) => relevantToQuery(query, hit)).slice(0, MAX_SOURCES);
}

export async function gatherResearch(topic: string): Promise<{ text: string; count: number; query: string }> {
