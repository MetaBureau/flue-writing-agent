...
export const STYLE_NAMES = ["economist", "strunk-white", "monocle", "professional"] as const;
...
export const styles: Record<StyleName, StyleRules> = {
  economist: {
    ...
    transformation: "Use active voice and short sentences. Cut praise, passion, mission statements, and repeated points. Keep names, numbers, and URLs",
...
export function styleFromData(data: { style?: string } | undefined): { key: StyleName; rules: StyleRules } {
...
export function styleSystemPrompt(style: StyleRules): string {
  return `Apply ${style.name} style rules: ${style.rules.join(", ")}. Transformation goal: ${style.transformation}. Example sentence: ${style.sample} Do not add facts.`;
}
