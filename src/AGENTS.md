...
- Fact-check calls `CHECK_MODEL`, or the form checker, via `HAIMAKER_API_KEY`. Default is `openai/gpt-4.1`. If that id matches the writer, or `CHECK_MODEL` is not a HaiMaker picker id, use another curated id and log the replacement. No HaiMaker key skips fact-check before research (`no HaiMaker key; not checked`) and still saves the essay. A checker auth or rate-limit error saves the essay marked unchecked, then reports the error. A cut-off or empty verdict list saves without citations. Unsupported and unchecked claims stay out of the essay
